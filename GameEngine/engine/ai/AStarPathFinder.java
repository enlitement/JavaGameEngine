...
public class AStarPathFinder implements PathFinder {
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance,
		...
		for (int tx = 0; tx < map.getWidthInTiles(); tx++)
			for (int ty = 0; ty < map.getHeightInTiles(); ty++)
	 ...
	 */
	public Path findPath(Mover mover, int gx, int gy, int sx, int sy, int tx,
			int ty) {
		// easy first check, if the destination is blocked, we can't get there

		/*if (map.blocked(mover, gx, gy, tx, ty)) {
			return null;
		}*/
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					...
					if (isValidLocation(mover, gx, gy, sx, sy, xp, yp)) {
	 ...
	 */
	protected boolean isValidLocation(Mover mover, int gx, int gy, int sx,
			int sy, int fx, int fy) {
		boolean invalid = (fx < 0)
				|| (fy < 0)
				|| gx < 0
				|| gy < 0
				|| (fx >= map.getWidthInTiles())
				|| (fy >= map.getHeightInTiles() || gx > map.getGridWidth() || gy > map
						.getGridHeight());

		if ((!invalid) && ((sx != fx) || (sy != fy))) {
			invalid = map.blocked(mover, gx, gy, fx, fy);
		}
	 ...
	 */
	public float getHeuristicCost(Mover mover, int sx, int sy, int fx, int fy) {
		return heuristic.getCost(map, mover, sx, sy, fx, fy);
	}
