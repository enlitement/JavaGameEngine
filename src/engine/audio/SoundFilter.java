...
/**
 * An abstract class designed to filter sound samples. Since SoundFilters may
 * use internal buffering of samples, a new SoundFilter object should be created
 * for every sound played. However, SoundFilters can be reused after they are
 * finished by calling the reset() method. Assumes all samples are 16-bit,
...
public abstract class SoundFilter {
	 ...
	 * Note that the offset and length are number of bytes, not samples.
	 */
